...
double myMin2(double a[], int left, int right) {
    double result = a[left];
...
void multi_prediction (SFC sfcs[], int server[], int nf_distribution[], double nf_result[], double sfc_result, int nfs_result[]) {
    ...
    for (int k=0;k < SFC_NUMBER;k++) {
        if (sfcs[k].node_id != 0 && sfcs[k].bottleneck >= sfcs[k].current && sfcs[k].bottleneck < sfcs[k].last && (server[0] - node0_core) > (sfcs[k].bottleneck - sfcs[k].current)) {
            ...
            //double result_tmp = myMin2(nf_result, 0, 3) + myMin2(nf_result, 3, 6) + myMin2(nf_result, 6, 9) + myMin2(nf_result, 9, 12) + myMin2(nf_result, 12, 15) + myMin2(nf_result, 15, 18);
            double result_tmp = myMin2(nf_result, 0, 3) + myMin2(nf_result, 3, 7) + myMin2(nf_result, 7, 12) + myMin2(nf_result, 12, 12+NEW_SFC);
...
map<double, int*> prediction_search (int nfs[]) {
    ...
    for (int i=7;i<12;i++) {
        ...
        sfcs[2].current = 7;
        sfcs[2].last = 11;
        sfcs[2].nfs[i-7] = nf_tmp;
    }
    //new chain
    for (int i=12;i<12+NEW_SFC;i++) {
        NF nf_tmp;
        nf_tmp.nf = nfs[i];
        nf_tmp.core = 0;
        sfcs[3].current = 12;
        sfcs[3].last = 12+NEW_SFC-1;
        sfcs[3].nfs[i-12] = nf_tmp;
    }
...
map<double, int*> greedy_placement(int nfs[]) {
    ...
    gettimeofday(&end, NULL);
    cout << "greedy placement1 run time is : " << ((end.tv_usec - start1.tv_usec) + (end.tv_sec - start1.tv_sec) * 1000000) << "us" << endl;
    cout << "greedy placement2 run time is : " << ((end.tv_usec - start2.tv_usec) + (end.tv_sec - start2.tv_sec) * 1000000)  << "us" << endl;
    map<double, int*> myMap;
    myMap[sfc_result] = nfs_result;
    return myMap;
}
...
int main() {
    ...
    int total_time_placement = 0;
    int total_time_greedy = 0;
    int hit_number = 0;
    int miss_number = 0;
    double miss_loss = 0;
    timeval start, end;
    ...
    for (int i=0;i<100;i++) {
        ...
        gettimeofday(&end, NULL);
        total_time_placement += ((end.tv_usec - start.tv_usec) + (end.tv_sec - start.tv_sec) * 1000000);

        gettimeofday(&start, NULL);
        map<double, int*> result_greedy = greedy_placement(nfs);
        gettimeofday(&end, NULL);
        total_time_greedy += ((end.tv_usec - start.tv_usec) + (end.tv_sec - start.tv_sec) * 1000000);

        a = result_search.begin();
        b = result_placement.begin();
        c = result_greedy.begin();
    ...
    cout << "exhaustive placement run time is : " << total_time_placement << "us" << endl;
    cout << "greedy placement run time is: " << total_time_greedy << "us" << endl;
